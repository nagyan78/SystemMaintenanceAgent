...
- 后端：`86 passed`，另有 1 个第三方 Starlette TestClient 弃用警告；
...
## 3.1 2026-07-12 智能增强接线状态

- 自适应规划已进入原有 `diagnosis_planning_node`，候选预算真实约束首批内容诊断，并记录 revision、decision 和 stop reason；当前仍是本地单工作流批次反馈，不是分布式持续调度器。
- `ModelService` 在启用 fallback 且备用端点可用时，通过受调用数和 Token 预算约束的 `ModelRouter` 执行主备调用。
- Agent 的节点详情、路径和子节点只读工具已进入 workflow/version scoped `ToolRegistry`，支持持久化缓存和超时。
- 低置信度、检测器分歧和 inconclusive 输出进入 `diagnosis_triage`，人工确认后才创建正式问题。
- 评价 API 不接受客户端上传 F1；服务端根据 workflow 结果和人工 Golden fixture 计算，baseline promote 写入审计日志。
- 删除叶子节点会查询 `category_reference`，不再信任模型 payload 中的引用数量。
- Qdrant 索引成功后递增 `vector_index_generation`，用于缓存失效。
