...
> 日期：2026-07-10
> 状态：终局设计已评审；阶段一已完成（2026-07-11），阶段二～四待实施
> 适用范围：在已完成 M1～M5 的基础上，将一次性 Excel 分析流水线升级为版本驱动、可恢复、可评价、可持续迭代的多智能体维护平台。
> 前置文档：`00_开发里程碑索引.md`、`10_LangGraph智能体工作流开发设计.md`、`11_智能体架构迭代优化设计.md`
...
阶段一冻结 `quality-v1`：只使用能够针对任意不可变版本重新计算的确定性指标，不读取某次 Agent 是否恰好写入问题来代替全量指标。六个维度及满分保持不变，但数据可用性必须显式记录：

| 维度 | quality-v1 可计算指标 | 缺失数据处理 |
|---|---|---|
| 结构完整性 25 | 缺失父节点、环、孤立节点、路径不一致率 | 分类树不可读则评价失败 |
| 层级与分支均衡度 20 | 超深节点率、过宽节点率、深度/宽度分布 | 分类树不可读则评价失败 |
| 父子语义一致性 20 | 已完成向量索引时的父子异常率 | 索引不可用则该维度 `available=false`，不伪造满分 |
| 节点冗余度 15 | 同级同名率；向量可用时增加语义重复率 | 向量部分不可用时记录 partial coverage |
| 命名与同义词规范性 10 | 空白、长度、非法分隔、名称/同义词重复等确定性规则 | 节点文本不可读则评价失败 |
| 诊断覆盖与可信度 10 | 本次实际执行的 detector coverage | 无记录时为 0，不把“未检测”解释为“无问题” |

`total_score` 采用可用维度原始得分之和，不对缺失维度重新归一化；同时持久化 `available_points`、`coverage_ratio` 和每个维度的 `available`。因此低覆盖评价不能与完整覆盖评价只看总分，Verification 的自动通过条件还要求 before/after 使用同一 `score_version` 且可用维度集合一致。`quality-v1` 发布后公式、阈值和 detector version 不原地修改；未来全量 screening 上线时发布 `quality-v2`。
...
flowchart TD
    ...
    SAVE --> REINDEX["结果版本重新索引\n阶段一完整兜底，后续增量"]
    REINDEX --> NEW_EVAL["新版本质量评价"]
    NEW_EVAL --> VERIFY["Verification / Critic Agent"]
    VERIFY_LOAD --> VERIFY_PREP["确保两版本索引与同版质量评价"]
    VERIFY_PREP --> VERIFY
    VERIFY --> DECIDE{是否达到目标}
    DECIDE -->|达标| REPORT
...
只运行基础版本/结果版本索引就绪检查、同一 `score_version` 质量评价、受影响范围复检和结果报告，不生成诊断建议，不进入人工审核与动作执行。若没有可用的动作 diff，调用方必须提供 `affected_node_ids`；两者都缺失时拒绝启动 verify workflow。
...
### 8.5 Interrupt 与 resume 契约

阶段一保留单一恢复端点 `POST /api/workflows/{task_id}/resume`，请求使用 `interrupt_type` 作为判别字段：
...
- `human_review` 只接受 `approve/reject/edit`，任务状态必须为 `waiting_review`。
- `continue_optimization` 只接受 `continue/finish`，任务状态必须为 `waiting_continue`。
- interrupt payload 和请求的 `interrupt_type` 必须一致；不一致返回 409，不恢复 Graph。
- 每次 interrupt 生成 `interrupt_id`；已消费的 `(task_id, interrupt_id)` 重复提交返回上次结果，不重复执行。
- `continue` 令 result version 成为下一轮 base/current version，`round += 1`，创建新 analysis run，然后回到 baseline evaluation 与 PLAN。
- `finish` 直接生成事实报告；达到 `max_rounds` 时不得接受 continue。
- 终态固定为 `completed`、`completed_degraded`、`failed`、`cancelled`；人工处理使用 `waiting_manual_intervention`，不是 completed。

### 8.6 Analysis run 与证据作用域

一个 workflow 可以包含多个维护轮次，每一轮必须创建独立的 `analysis_run_id`。版本是被分析对象，analysis run 是诊断事实的发生上下文，两者不能混用。

阶段一最小约束：

- `analysis_run` 保存 `workflow_id`、`round`、`analyzed_version_id`、状态和起止时间，唯一键为 `(workflow_id, round)`。
- `diagnosis_issue`、`adjustment_suggestion`、`operation_log` 必须可追溯到 `workflow_id` 和 `analysis_run_id`；建议还必须通过 `issue_id` 归属同一 run。
- 同一版本被不同 workflow 或不同轮次分析时，不覆盖历史证据。
- 结构诊断的“替换”语义只允许替换当前 analysis run 的同 detector 输出，禁止按 `version_id` 删除其他 run 的问题。
- 报告、质量解释和 Verification 只读取当前 workflow/run 的事实；跨版本稳定问题比较使用 fingerprint，不用数据库行 ID 作为身份。
- 进入下一轮时生成新的 `analysis_run_id`，清空 Graph State 中上一轮 review/action 引用，但保留历史记录。
...
交付：workflow mode、base version、version lineage、analysis-run/evidence-scope、QualityEvaluationAgent、VerificationAgent、结果版本重新索引（阶段一允许完整重建兜底）、继续优化入口；PLAN 复用现有 `diagnosis_planning_node` 的单次静态规划实现。

实施计划：`13_阶段一_版本持续维护闭环实施计划.md`

完成状态（2026-07-11）：已交付三模式 workflow、analysis-run/evidence-scope、版本 lineage、`quality-v1`、结果版本完整重建索引兜底、对称范围 Verification、continue resume、明确终态、作用域报告和前端质量对比。阶段验收为后端 94 tests passed，前端 contract test 与 production build 通过。
