...
sequenceDiagram
    ...
    LG->>DB: 保存建议
    U->>FE: 审核建议
    FE->>API: POST /api/suggestions/approve
    API->>LG: 执行确认后的动作
    LG->>DB: 更新节点并生成新版本
    LG->>FE: 返回执行结果
```
...
| LangGraph 节点 | 职责 |
|---|---|
| `parse_excel_node` | 读取 Excel，识别字段，转为节点列表 |
| `build_tree_node` | 构建父子关系、计算层级、路径、叶子节点 |
| `save_initial_version_node` | 保存原始版本和节点快照 |
| `index_vector_node` | 将节点文本向量化并写入 Qdrant |
| `structure_diagnosis_node` | 规则检测结构问题 |
| `content_diagnosis_node` | 语义检索 + LLM 判断内容问题 |
| `generate_suggestion_node` | 根据问题生成结构化建议 |
| `wait_human_review_node` | 暂停流程等待用户确认 |
| `validate_action_node` | 校验建议动作是否合法 |
| `execute_action_node` | 执行用户确认后的动作 |
| `save_new_version_node` | 保存新版本快照 |
| `generate_report_node` | 生成诊断报告 |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/files/upload` | 上传 Excel |
...
| POST | `/api/suggestions/{id}/approve` | 接受建议 |
| POST | `/api/suggestions/{id}/reject` | 拒绝建议 |
| PUT | `/api/suggestions/{id}` | 编辑建议 |
| POST | `/api/suggestions/execute` | 执行已确认建议 |
