<template>
  <AppShell><div class="page-stack">
    ...
    <section v-if="enableAi" class="card"><div class="card-head"><div><p class="eyebrow">智能预算</p><h2>计划与资源使用</h2></div><span class="badge">Revision {{ planRevision }}</span></div><div class="review-stats"><span class="badge">Model calls {{ modelCallsUsed }}</span><span class="badge">Token {{ tokensUsed }}</span><span class="badge">Wall time {{ wallSecondsUsed.toFixed(1) }}s</span><span class="badge">Batch decision {{ planDecision }}</span><span class="badge">Triage {{ triageCount }}</span></div><p v-if="stopReason">停止原因：{{ stopReason }}</p></section>
...
const planRevision=ref(1),planDecision=ref('initial'),stopReason=ref(''),modelCallsUsed=ref(0),tokensUsed=ref(0),wallSecondsUsed=ref(0),triageCount=ref(0)
