...
> **技术架构参考（REFERENCE）**：基础分层和技术边界仍可参考，但其中的阶段状态、路线和代码清单可能过期。当前事实与路线分别见[当前实现情况](../00_当前状态/当前实现情况.md)、[当前开发路线图](../00_当前状态/当前开发路线图.md)。
...
| 自动维护进度页 | StepTimeline、TaskStatusBar、自动复核与执行摘要 |
...
1. 统一封装 DeepSeek LLM 调用（`ChatOpenAI`）。
...
> **历史修订说明**（2026-07-05）：当时曾以 M1-M5 替换原阶段划分，旧索引现归档于[开发里程碑索引](../99_历史归档/旧里程碑/开发里程碑索引.md)。当前路线见[当前开发路线图](../00_当前状态/当前开发路线图.md)。
