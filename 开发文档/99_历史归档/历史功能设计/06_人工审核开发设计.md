...
> `queued`、`executed`、`failed` 属于 F07/M4 动作执行状态，M3 不进入这些状态。
...
### 5.5 Workflow 审核闭环（M3 主路径）

M3 的主路径通过 LangGraph interrupt/resume 完成审核，前端或调用方按批次提交一次审核决策：
...
`/api/suggestions/*` 单条审核接口可作为辅助查询和调试接口，但 M3 验收优先以 workflow resume 是否恢复到 `validate_action_node` 为准。
...
| 拒绝 | 当前状态必须是 `pending` 或 `edited` |
| 编辑 | 当前状态必须是 `pending` 或 `edited` |
| 批量接受 | 所有建议必须为 `risk_level=low` 且状态为 `pending` |

编辑后的建议还必须重新执行 F05 中的结构化校验，确保动作可被 F07/M4 执行校验识别。
