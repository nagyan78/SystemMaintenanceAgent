...
def create_evaluation(payload:CreateEvaluationRequest,request:Request):
    ...
    with connect(request.app.state.settings) as c:
        ...
        triage_count=int(c.execute("SELECT COUNT(*) FROM diagnosis_triage WHERE workflow_id=?",(payload.workflow_id,)).fetchone()[0])
    ...
    result=result.model_copy(update={"triage_count":triage_count})
