...
class Settings(BaseModel):
    ...
    llm_max_tokens: int = Field(default_factory=lambda: int(os.getenv("LLM_MAX_TOKENS", "10000000")))
    llm_request_timeout_seconds: int = Field(default_factory=lambda: int(os.getenv("LLM_REQUEST_TIMEOUT_SECONDS", "45")))
    diagnosis_ai_candidate_limit: int = Field(default_factory=lambda: int(os.getenv("DIAGNOSIS_AI_CANDIDATE_LIMIT", "50")))
    diagnosis_sample_size: int = Field(default_factory=lambda: int(os.getenv("DIAGNOSIS_SAMPLE_SIZE", "50")))
