...
def test_review_feedback_becomes_scoped_memory(tmp_path):
    ...
    context=AgentMemoryService(settings).get_suggestion_context(version_id=version_id,issue_type="semantic_duplicate",action_type="mark_as_valid")
...
def test_golden_metrics_release_gate_and_api(tmp_path):
    ...
    settings=_settings(tmp_path); client=TestClient(create_app(settings)); created=client.post("/api/evaluations",json={"result":result.model_dump(),"agent_bundle_version":"bundle-1"})
